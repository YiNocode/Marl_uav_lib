...
def test_merge_task_with_env_scenario_defaults():
    env_cfg_path = ROOT / "configs/env/pyflyt_obstacles.yaml"
    ...
    assert merged["obstacle_grid_spacing"] == 10.0
...
def test_experiment_override_wins_over_env_defaults():
    env_cfg = {
        "scenario_config": "configs/env/scenarios/e2_obstacle_field.yaml",
    }
...
def test_build_env_from_e2_config_applies_scenario_defaults():
    env = build_env_from_config(
        ROOT / "configs/env/pyflyt_obstacles.yaml",
        seed=101,
        task_cfg={"pursuer_speed": 0.25, "evader_speed": 0.25},
    )
    try:
        assert env.task.obstacle_grid_spacing == 10.0
        assert env.task.world_xy == 20.0
        assert env.task.num_obstacles_min == 25
    finally:
        env.close()

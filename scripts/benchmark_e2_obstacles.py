"""E2 obstacle-rich benchmark for heuristic baselines in obstacle fields.

Evaluates geometric / slot / trajectory-planner heuristics on PyFlyt ex2
(cylindrical obstacles) and writes task + structure + obstacle metrics.
"""
...
def _build_heuristic_worker(cfg_path: Path, seed: int) -> RolloutWorker:
    ...
    if "fixed_ring" in cfg:
        ...
        raise ValueError(
            ...
            "fixed_ring, pure_pursuit, oracle_slot, hungarian_slot, ot_slot, trajectory_planner."
        )
    ...
    *,
    ...
    for method in methods:
        ...
        if method_kind != "heuristic":
            raise ValueError(
                f"E2 benchmark is heuristic-only; method={method!r} has kind={method_kind!r}."
            )
        ...
        for seed in eval_seeds:
            ...
            print(f"[eval] method={method} ({paper}) eval_seed={seed} episodes={episodes}")
            try:
                ...
                try:
                    ...
                    pass
            csv_name = f"{method}_eval{seed}_records.csv"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run E2 obstacle-rich PyFlyt heuristic benchmark.")
    p.add_argument(
        "--suite-config",
        type=str,
        default="configs/benchmark/e2_obstacles_suite.yaml",
    )

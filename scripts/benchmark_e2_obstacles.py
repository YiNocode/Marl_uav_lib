"""E2 obstacle-rich benchmark: structure-preserving encirclement vs. baselines in obstacle fields.

Compares framework instances (dream_mappo_full), RL execution backends (mappo, …),
and geometric heuristics on PyFlyt ex2 (cylindrical obstacles).

- generates per-method/per-seed configs under configs/generated/e2_obstacles/
- trains RL methods via scripts/train.py
- evaluates checkpoints and heuristics on PyFlyt
- writes task + structure + obstacle metrics to results/e2_obstacles_pyflyt/
"""
...
def generated_config_path(suite: dict[str, Any], method: str, seed: int) -> Path:
    gen_dir = ROOT / str(suite.get("generated_config_dir", "configs/generated/e2_obstacles"))
    ...
    *,
    ...
    if not ckpt.is_file():
        raise FileNotFoundError(
            ...
            "Run benchmark_e2_obstacles.py --mode train first."
        )

"""Rate-limited commanded-slot generation for dynamic slot jumps."""
...
from experiments.slot_tracking.controllers.baseline_pure_pursuit import clip_norm
from experiments.slot_tracking.controllers.grid_path_planner import plan_grid_astar
...
class SlotTransitionManager:
    """Convert raw slots into valid, trackable commanded slots.

    The manager preserves raw-slot diagnostics while giving controllers a
    physically plausible reference: continuous valid raw slots pass through,
    jumps and invalid raw slots are projected and rate-limited.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.slot_ref_vmax = float(slot_ref_vmax)
        self.slot_ref_amax = float(slot_ref_amax)
        self.jump_detection_threshold = float(jump_detection_threshold)
    ...
    def reset(self, raw_slot_pos: np.ndarray, obstacles: list[Any]) -> dict[str, Any]:
        ...
        commanded_validity = self._validity_diag(proxy, obstacles)
        return self._output(
            raw=raw,
            proxy=proxy,
            ...
            commanded_slot_valid=bool(commanded_validity["commanded_slot_valid"]),
    ...
    def update(
        ...
        *,
        ...
        minimum_reach_time = raw_delta / max(self.slot_ref_vmax, 1e-9)
        ...
        command_gap = float(np.linalg.norm(proxy[:2] - prev_cmd[:2]))
        rejoin_directly = bool(command_gap <= max(self.slot_ref_vmax * self.dt, 0.03) + 1e-9)
        hold_unstable = bool(raw_slot_too_unstable or int(step) < int(self.state.unstable_hold_until_step))
        if bool(diag["raw_slot_valid"]) and not jump_detected and self.state.mode not in ("JUMP_INTERPOLATION", "PROXY_INTERPOLATION", "HOLD_UNSTABLE", "SAFE_HOLD") and rejoin_directly:
        ...
        ) and bool(diag.get("proxy_slot_valid", False)):
    ...
    def _limited_step(self, current_xy: np.ndarray, current_vel_xy: np.ndarray, target_xy: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        delta = np.asarray(target_xy, dtype=np.float64).reshape(2) - np.asarray(current_xy, dtype=np.float64).reshape(2)
        desired_vel = clip_norm(delta / self.dt, self.slot_ref_vmax)
        dv = clip_norm(desired_vel - np.asarray(current_vel_xy, dtype=np.float64).reshape(2), self.slot_ref_amax * self.dt)
        next_vel = clip_norm(np.asarray(current_vel_xy, dtype=np.float64).reshape(2) + dv, self.slot_ref_vmax)
    ...
    def _project_to_valid(self, raw_slot: np.ndarray, obstacles: list[Any]) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        raw_outside = bool(np.max(np.abs(raw[:2])) > self.world_xy)
    ...
    def _is_valid(self, slot: np.ndarray, obstacles: list[Any]) -> bool:
        ...
        return bool(np.max(np.abs(p[:2])) <= self.world_xy and self._clearance(p[:2], obstacles) >= self.safety_margin - 1e-6)

    def _validity_diag(self, slot: np.ndarray, obstacles: list[Any]) -> dict[str, Any]:
        p = np.asarray(slot, dtype=np.float64).reshape(3)
        outside = bool(np.max(np.abs(p[:2])) > self.world_xy - self.uav_radius - self.safety_margin + 1e-6)
    ...
    def _segment_safe(self, start: np.ndarray, end: np.ndarray, obstacles: list[Any]) -> bool:
        ...
        for t in np.linspace(0.0, 1.0, 9):
            p = a + float(t) * (b - a)
            if np.max(np.abs(p)) > self.world_xy - self.uav_radius - self.safety_margin + 1e-6:
                return False
    ...
    def _planned_command_step(self, start: np.ndarray, target: np.ndarray, obstacles: list[Any]) -> tuple[np.ndarray, bool, str]:
        start_xy = np.asarray(start, dtype=np.float64).reshape(3)[:2]
        target_xy = np.asarray(target, dtype=np.float64).reshape(3)[:2]
        path, diag = plan_grid_astar(
            start_xy,
            target_xy,
            obstacles,
        ...
        if not bool(diag.get("planner_success", False)):
            return np.asarray(start, dtype=np.float64).reshape(3).copy(), False, str(diag.get("planner_reason", "NO_PATH"))
        pts = np.asarray(path, dtype=np.float64).reshape(-1, 2)
        if pts.shape[0] < 2:
            out = np.asarray(target, dtype=np.float64).reshape(3).copy()
            return out, True, "ALREADY_AT_TARGET"
        ...
        for nxt in pts[1:]:
            ...
            if seg_len <= 1e-9:
                ...
                continue
            if budget <= seg_len:
                xy = cursor + seg * (budget / seg_len)
                out = np.asarray(start, dtype=np.float64).reshape(3).copy()
                out[:2] = xy
                out[2] = float(np.asarray(target, dtype=np.float64).reshape(3)[2])
                return out, True, "PATH_COMMAND_STEP"
            budget -= seg_len
            cursor = nxt.copy()
        out = np.asarray(target, dtype=np.float64).reshape(3).copy()
        return out, True, "PATH_COMMAND_TARGET"
